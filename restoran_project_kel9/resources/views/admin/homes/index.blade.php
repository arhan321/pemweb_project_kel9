                ...
                <tbody>
                    @foreach($home as $key => $h)
                        <tr data-entry-id="{{ $h->id }}">
                            ...
                            <td>
                                @if($h->image)
                                    <a href="{{ $h->image->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $h->image->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @can('home_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.homes.show', $h->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('home_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.homes.edit', $h->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('home_delete')
                                    <form action="{{ route('admin.homes.destroy', $h->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
